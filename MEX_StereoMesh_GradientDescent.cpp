...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
	...
	int dims_img_out[3], dims_v_energy[2], dims_D_out[2];
	...
	plhs[1] = mxCreateNumericArray(3, dims_img_out, mxDOUBLE_CLASS, mxREAL);
	final_img2_interp = (double *)mxGetPr(plhs[1]);

	dims_v_energy[0] = N_ITERmax + 1;
	dims_v_energy[1] = 1;
	plhs[2] = mxCreateNumericArray(2, dims_v_energy, mxDOUBLE_CLASS, mxREAL);
	v_energy = (double *)mxGetPr(plhs[2]);

	dims_D_out[0] = 3 * N_T;
	dims_D_out[1] = 1;
	plhs[3] = mxCreateNumericArray(2, dims_D_out, mxDOUBLE_CLASS, mxREAL);
	D_out = (double *)mxGetPr(plhs[3]);
	...
	StereoMesh_GradientDescent(img1, img2, grad_img2_x, width_img, height_img, K, Baseline, img_label, N_V, N_E, N_T, ind_vertex_in_triangle, ind_triangles_using_vertex,
									ind_triangles_using_edge, D_init, S, delta_init, lambda_BREACH, lambda_NORMAL, N_ITERmax, final_disparity_map, final_img2_interp, v_energy, D_out);
